...
import { distance, tourDistance, algo, googAlgo } from './traveling_salesman_algorithm.js';
...
$(() => {
  ...
  $('#presets').change(function() {
    ...
    if ($(this).val() == 1) {
      map.setZoom(11);
      map.setCenter({lat: 33.888398319723, lng: -5.55572356374744})
    }
    else if($(this).val() == 2){
      map.setZoom(11);
      map.setCenter({lat: 33.56977741950461, lng: -7.599677196572687})
    } else {
      map.setZoom(2);
      map.setCenter({lat: 33.69543895958896, lng: -7.3681910822576775})
    }
  });
...
function initMap() {
  ...
  let map = new window.google.maps.Map(document.getElementById('map'), {
    zoom: 14,
    center: {lat: 33.888398319723, lng: -5.55572356374744},
    scrollwheel: true
  });
  return map;
}
